...
for GOARCH in 386 amd64; do
  for GOOS in darwin linux windows freebsd; do
    ...
    go build -mod=vendor -o build/bin/logwatcher-$GOOS-$GOARCH -a -ldflags "-extldflags -static" -tags netgo -installsuffix netgo main/logwatcher_server.go
    ldd build/bin/logwatcher-$GOOS-$GOARCH
  done
done
